...
const int leftPickMagnetSwitch = 23; // Pin connected to reed switch
const int rightPickMagnetSwitch = 25; // Pin connected to reed switch
...
void shedding()  { 
  ...
  if (currentPick == 1) {
    ...
    moveBackBank(strainRegulationFirstPick[0]); //(+) = away from harnesses
    //Second increment
    moveH1(dirMultiplier * h1FirstPick[1]);
    moveH2(dirMultiplier * -h2FirstPick[1]);
    moveBackBank(strainRegulationFirstPick[1]);
...
void checkMagnetSwitches() {
  ...
  // You might need to adjust the delay or handling based on your application needs
  delay(1000); // Add a short delay to reduce serial monitor flooding
}
