...
int scoochDistance = 13;   // Steps for tensioner to travel 1.6 fiber diameters
...
void loop() {
  ...
  if (!weavingActive && currentPick == 1) {
      Reedhomemin(-1000);
      Fronthomemin(10000);
      Backhomemin(-10000);
      H1homemin(10000);
  ...
  //check for confirmation, using front bank limit switch
  limitSwitchFront.loop();
  int state = limitSwitchFront.getState();

  if (state == HIGH && homed == true) {
      weavingActive = true;
  }
...
void sheddingThreeSteps()  { 
  ...
  if (harnessesOut == false) {
    //First Increment 
    moveH1(dirMultiplier * -h1StepsArray[1]); //h1 and h2 move in opposite directions, (-) is up
