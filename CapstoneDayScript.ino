...
//                                                                                                                    
//                                                                                                                   
//         888888 88""Yb  dP"Yb  88b 88 888888                     πππππππππππππππ                    88""Yb    db     dP""b8 88  dP 
//         88__   88__dP dP   Yb 88Yb88   88                       ππ  H1 √√ H2 ππ                    88__dP   dPYb   dP   `" 88odP
//         88""   88"Yb  Yb   dP 88 Y88   88                       ππ  H1 √√ H2 ππ                    88""Yb  dP__Yb  Yb      88"Yb  
//         88     88  Yb  YbodP  88  Y8   88                       ππ  H1 √√ H2 ππ                    88oodP dP""""Yb  YboodP 88  Yb                               
//                                                                 ππ  H1 √√ H2 ππ                                                   
//                                            ∞                    ππ  H1 √√ H2 ππ                                                   
//                                            ∞∞       |======|    ππ  H1 √√ H2 ππ                                                  
//                                             ∞∞      |NEMA14|    ππ  H1 √√ H2 ππ                                                  
...
ezButton toggleSwitch(22); // Assuming pin 22 for the toggle switch
...
bool isMachinePaused = false; // To track pause state
...
int currentPick = 1;   //Starts as 1, increments based on total picks
...
void setup() {
 ...
 // Initialization of SPI/driver functions. Detailed descriptions here -> https://pololu.github.io/drv8434s-arduino/class_d_r_v8434_s.html#ac9909297f589d0a3660535e22ec2e11f
  ...
  pinMode(leftPickMagnetSwitch, INPUT_PULLUP);
  pinMode(rightPickMagnetSwitch, INPUT_PULLUP);
  toggleSwitch.setDebounceTime(50); // Set debounce time to 50 milliseconds for the toggle switch
...
void loop() {
  // First, check the state of the toggle switch
  toggleSwitch.loop(); // MUST call the loop() function first for ezButton
  
  // Toggle the pause state when the switch is pressed
  if (toggleSwitch.isPressed()) {
    isMachinePaused = !isMachinePaused; // Toggle the pause state
    if (isMachinePaused) {
  ...
  // If the machine is paused, skip the rest of the loop iteration
  if (isMachinePaused) {
    return; // Early return to pause the machine
  }
  checkMagnetSwitches();
  //HOMING INSTRUCTIONS
...
void checkMagnetSwitches() {
  int leftProximity = digitalRead(leftPickMagnetSwitch); // Read the state of the left switch
  int rightProximity = digitalRead(rightPickMagnetSwitch); // Read the state of the right switch
  
  // If the pin reads LOW, the switch is closed (magnet is near).
  if (leftProximity == LOW) {
    Serial.println("Left Switch closed - Magnet detected");
  }
  else {
    Serial.println("Left Switch opened - Magnet not detected");
  }
  
  // Similar logic for the right switch
  if (rightProximity == LOW) {
    Serial.println("Right Switch closed - Magnet detected");
  }
  else {
    Serial.println("Right Switch opened - Magnet not detected");
  }
  delay(500); // Add a short delay to reduce serial monitor flooding
}
